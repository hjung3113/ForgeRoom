...
import type { OrchestratorFailureCode } from '../core/errors.js';
import {
...
describe('SqliteTaskStore', () => {
  ...
  it('defaults mastra_run_id to null and round-trips set and clear (ADR-017)', async () => {
    await store.createTask(taskInput('task-mastra', 'project-a', 'queued'));

    const created = await store.getTask('task-mastra');
    expect(created?.mastra_run_id).toBeNull();
    await expect(store.getMastraRunId('task-mastra')).resolves.toBeNull();
    expect(
      database.sqlite.prepare('select mastra_run_id from tasks where id = ?').get('task-mastra'),
    ).toEqual({ mastra_run_id: null });

    await store.setMastraRunId('task-mastra', 'mastra-run-123');
    await expect(store.getMastraRunId('task-mastra')).resolves.toBe('mastra-run-123');
    await expect(store.getTask('task-mastra')).resolves.toMatchObject({
      mastra_run_id: 'mastra-run-123',
    });
  ...
  it('accepts an explicit mastra_run_id at task creation', async () => {
    await store.createTask({
      ...taskInput('task-mastra-create', 'project-b', 'queued'),
      mastra_run_id: 'mastra-run-create',
    });

    await expect(store.getTask('task-mastra-create')).resolves.toMatchObject({
      mastra_run_id: 'mastra-run-create',
    });
  });
  ...
  it('upserts conductor state and preserves last_step_id on summary-only refreshes', async () => {
    ...
    // Summary-only refresh (lastStepId omitted) must keep the existing last_step_id.
    ...
    vi.setSystemTime(new Date('2026-05-22T13:23:00.000Z'));
    await store.upsertConductorState(
      'task-conductor-state',
      'Cleared step',
      '/tmp/forgeroom/context/summary-v4.md',
      null,
    );
    await expect(store.getConductorState('task-conductor-state')).resolves.toMatchObject({
      last_step_id: null,
  ...
  it('rejects non-canonical failure reasons on every persistence path', async () => {
    await store.createTask({
  ...
  it('updates task status with a canonical failure reason and rejects non-canonical', async () => {
  ...
  it('rolls back task creation when starting a task cannot acquire the project lock', async () => {
    ...
    expect(
      database.sqlite.prepare('select count(*) as count from tasks where id = ?').get('loser'),
    ).toEqual({
      count: 0,
    });
  });
  ...
  it('rolls back task cancellation when the task_canceled event cannot be inserted', async () => {
    ...
    await expect(
      store.cancelTask('task-cancel-rollback', 'duplicate-cancel-event', {
        reason: 'user_request',
      }),
    ).rejects.toThrow(/unique constraint/i);
    ...
    await expect(store.acquireProjectLock('project-a', 'task-blocked-by-rollback')).resolves.toBe(
      false,
    );
