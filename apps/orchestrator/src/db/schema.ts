...
import type { CheckStatus, ExternalRef, StepStatus, TaskSource, TaskStatus } from '../core/types.js';
...
export const tasks = sqliteTable(
  'tasks',
  {
    ...
    // mastra_run_id (ADR-017): TaskStore step rows are authoritative; the Mastra
    // workflow run snapshot is auxiliary. recoverPending() reads this nullable
    // pointer to decide whether to call Mastra run.resume() (id present and the
    // suspended run matches the TaskStore next-step pointer) or to start a fresh
    // Mastra run reconstructed from TaskStore + .forgeroom/ files (id null or
    // mismatch). It is never queried by value, so it carries no index.
