CREATE TABLE IF NOT EXISTS tasks (
  ...
  pr_number INTEGER,
  final_slices TEXT NOT NULL,
  vars TEXT NOT NULL,
  -- mastra_run_id (ADR-017): nullable pointer from the authoritative TaskStore
  -- task row to the auxiliary Mastra workflow run snapshot. recoverPending()
  -- reads it to choose Mastra run.resume() vs. starting a fresh reconstructed
  -- run. Never queried by value, so it has no index.
  mastra_run_id TEXT,
