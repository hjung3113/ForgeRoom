...
| OQ-M01 | Mastra `.dountil()` step body가 iteration index를 직접 노출하는가? 노출 안 되면 어댑터가 input/output으로 thread해야 함 | review_loop iteration 추적, 파일명 규칙 (`07_slice_review.0.md`) 유지 | **결과 (b)**: step body에 노출 안 됨 (`iterationCount`는 `.dountil()` condition 전용). 어댑터가 input/output으로 manual thread하며, mid-loop suspend/resume에서도 counter 보존됨을 spike로 확인. `@mastra/core@1.36.0` 기준. | resolved by spike `Docs/spikes/2026-05-23-oq-m01-dountil-iteration.md` (#2, codex confidence 92) |
| OQ-M02 | Mastra가 `.foreach()` mid-iteration에서 suspend 시 snapshot이 깔끔히 복구되는가? | `pause_after`가 foreach 내부 step에서 트리거될 때 동작 | **outcome (a) NATIVE**: sequential `.foreach()` (concurrency 1)은 mid-iteration suspend/resume을 native 지원. 어댑터 lowering·DSL 제약 불필요. 영속 snapshot store 필요(이미 충족). 스파이크: `Docs/spikes/2026-05-23-oq-m02-foreach-suspend.md`, 코드 `apps/orchestrator/spikes/oq-m02/foreach-suspend.spike.ts` | resolved (spike 2026-05-23, codex confidence 84) |
| OQ-M03 | Mastra 버전 안정성 (1.0 이전이면 minor pin + 회귀 테스트 전략) | upgrade 비용, MVP stability | Mastra 버전 확정 시점에 lock strategy 결정 | pending |
| OQ-M04 | Mastra Studio 트레이스가 외부 CLI agent process (OpenClaw subprocess) I/O를 포함하는가? 포함 안 되면 step linkage만 가능 | Studio가 실제 디버깅에 얼마나 유용한지 | **결과: step-boundary만 (subprocess stdio 미포함).** Mastra 트레이스는 step/agent/tool/model span의 input·output·timing·token usage만 기록한다. OpenClawProvider는 CLI subprocess의 stdout/stderr를 step body 안에서 `.forgeroom/logs/<task>.{stdout,stderr}` 파일로 redirect하므로 Mastra가 그 바이트를 보지 못한다. step body가 반환하는 구조화된 결과(output text, `.forgeroom/` 경로 참조)만 trace에 들어간다. #10에서 실측: Studio API run snapshot이 step별 `output: {outputPath: ".forgeroom/outputs/impl_plan.md", ...}` (경로 참조)만 담고 stdio는 없음을 확인. 따라서 Studio = step linkage + 구조화 I/O 디버깅; raw subprocess 로그는 `.forgeroom/logs/`에서 본다 (`Docs/dev/studio.md` 참조). | resolved (#10, codex confidence 88; Mastra observability docs + 실측 run snapshot) |
