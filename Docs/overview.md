...
장기적으로 ForgeRoom은 agent workflow orchestration과 RAG 기반 prompt optimization을 결합한 시스템을 지향한다. Conductor는 task-local context를 정리해 다음 agent에게 더 좋은 프롬프트를 제공하고, Forge Phase 2 이후에는 Target Profile, project docs, ADR, issue/PR history 같은 외부 지식까지 연결하는 ContextProvider 계층으로 확장할 수 있다.
