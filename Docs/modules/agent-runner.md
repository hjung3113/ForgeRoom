...
- **bootstrap artifact 격리(#124)**: workspace=worktree이므로 OpenClaw가 persona 파일(`SOUL.md`/`IDENTITY.md`/`TOOLS.md`/`USER.md`/`HEARTBEAT.md`/`.openclaw/`)을 worktree 루트에 쓴다. `ensure`는 agent 생성 직후 이 denylist(`OPENCLAW_ARTIFACT_EXCLUDES`)를 worktree의 로컬 `info/exclude`에 idempotent하게 기록해 `git add --all`이 건너뛰게 한다. denylist는 OpenClaw 어댑터 소유, exclude 메커니즘은 provider-neutral(`GitCli.excludeFromWorktree`) — generic commit 경로는 OpenClaw 파일 이름을 모른다. 타겟 repo의 tracked `.gitignore`에는 쓰지 않는다. (ADR-030 보강)
