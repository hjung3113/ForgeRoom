...
11. .forgeroom/prompts/NN_<step_id>.md 에 renderer-owned base prompt 저장 (실행 프롬프트는 renderer 소유; ADR-027 #121 amendment)
12. Conductor.refineNotes(task_id, step_id, base_prompt) → 컨텍스트 노트를 .forgeroom/context/refined-notes/NN_<step_id>.md 에 별도 stage (비거나 실패 시 생략). 프롬프트를 대체하지 않는다
13. AgentRunner.run({
...
- **Bundled template root**는 `FORGEROOM_TEMPLATE_ROOT`로 지정하며 기본값은 `<repo>/templates` (configs와 형제). composition root가 `templateRoot`를 PipelineEngine에 주입하고, boot 경로가 같은 root로 `templateExists`를 wiring해 WorkflowRegistry validation이 실제로 동작한다.
- **변수 치환**: 템플릿 본문의 `{{key}}` placeholder를 치환한다. source는 step의 보간된 `input_refs` + `vars` (DSL `${...}` 레이어는 그 전에 이미 평가됨) + `{{step_id}}` + `{{step_index}}`. 알 수 없는 `{{key}}`는 fail-fast(`agent_error`)다 — 깨진 프롬프트를 조용히 내보내지 않는다. `${...}`는 템플릿 안에서 지원하지 않는다(그건 DSL 레이어).
- **Harness 계약 로딩(step 8의 harness 부분)**: run step의 harness id로 `configs/harnesses.yaml`의 worktree-relative source를 찾아 `<worktree>/<source>`에서 staged 계약을 읽고, step template과 같은 `{{}}` 규칙으로 보간한 뒤 `# Harness Contract` / `---` / `# Step Prompt` 형식으로 합성한다. 이 composed base가 renderer-owned 프롬프트(step 11)이자 refineNotes(step 12)의 입력이다. harness가 null인 step은 template-only다. Bundled 계약은 `<repo>/harnesses`에 있고 WorktreeManager가 worktree `.forgeroom/harnesses/<id>`로 stage한다([ADR-027](../decisions/2026-05-25-027-harness-contract-staging.md)).
