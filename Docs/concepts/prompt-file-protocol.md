...
│   ├── target-profile.md    # ForgeRoom이 관리하는 Target Profile의 실행 시점 snapshot
...
3. Intent resolve: step의 `intent`를 `configs/intents.yaml`에서 찾아 Resolved Step 생성
4. Resolved Step의 harness prompt/output contract + step의 prompt template 로드 (`<bundled_template_root>/<prompt_template>`)
5. 변수 보간 (${...} 치환)
6. 미반영 user_feedback event가 있으면 Conductor.integrateFeedback(task_id) → context/feedback.md 갱신
7. Conductor.refine(task_id, step_id, base_prompt) → summary.md + feedback.md + 직전 결과 기반 보강 프롬프트
8. .forgeroom/prompts/NN_<step_id>.md 에 저장
9. AgentRunner.run({
...
11. Resolved Step의 `kind`가 `execute`이면 CheckRunner 실행
   - 실패 시 마지막 코드 작성 agent에 1회 자동 수정 요청
   - 수정 후 모든 check 재실행
   - 재실패 시 step.status=failed
12. git diff → diffs/NN_<step_id>.diff 저장
13. step.status=done
14. Conductor.update(task_id, stepResult) — 동기 호출
15. Reporter.notify(step_done) — 비동기 (events 테이블 우선 기록)
```
...
- slice가 0개면 PipelineEngine의 output selector 해석 단계에서 검증 실패로 간주하고, 같은 session에 유효한 `## Slices` 섹션으로 다시 작성하라고 resume 요청한다. 이 재시도는 output 파일 미작성과 같은 resume budget(최대 2회)을 사용한다.
- `kind: review` step은 output의 첫 non-empty line에 `Review Result: pass` 또는 `Review Result: fail`을 써야 한다.
- review result 헤더가 없거나 다른 값이면 PipelineEngine의 output selector 해석 단계에서 검증 실패로 간주하고, 같은 session에 올바른 헤더로 다시 작성하라고 resume 요청한다.
...
- Conductor는 같은 `.forgeroom/` 디렉토리 읽기. `context/target-profile.md`는 ForgeRoom이 staging한 Target Profile snapshot이며, Conductor가 갱신하지 않는다. summary 갱신 시 `context/summary.md`, 피드백 통합 시 `context/feedback.md` write 만 허용
