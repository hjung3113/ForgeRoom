...
- **Harness 계약 로딩(step 8의 harness 부분)**: run step의 harness id로 `configs/harnesses.yaml`의 worktree-relative source를 찾아 `<worktree>/<source>`에서 staged 계약을 읽고, step template과 같은 `{{}}` 규칙으로 보간한 뒤 `# Harness Contract` / `---` / `# Step Prompt` 형식으로 합성한다. 이 composed base가 refine(step 11)의 입력이다. harness가 null인 step은 template-only다. Bundled 계약은 `<repo>/harnesses`에 있고 WorktreeManager가 worktree `.forgeroom/harnesses/<id>`로 stage한다([ADR-027](../decisions/2026-05-25-027-harness-contract-staging.md)).
