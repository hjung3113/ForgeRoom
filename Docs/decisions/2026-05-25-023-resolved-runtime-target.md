...
## 비범위
- OpenClawProvider 이전(scope B) — **완료 (#71, 2026-05-25)**: 아래 후속 노트 참조
- RuntimeCapabilities / capability matrix
- providerType union, per-target agentId/endpoint/token override

## 후속: scope B 완료 (#71, 2026-05-25)

scope A에서 deferred했던 provider relocation을 #71에서 마무리했다.

- `OpenClawProvider` 클래스와 OpenClaw 전용 transport 타입(`OpenClawExecutionRequest`,
  `OpenClawResumeRequest`, `OpenClawRunResponse`, `OpenClawHealthRequest`,
  `OpenClawIpcClient`, `OpenClawProviderConfig`)을
  `core/agent-runtime/openclaw-provider.ts` → `app/openclaw-provider.ts`로 이전
  (테스트도 `app/openclaw-provider.test.ts`로 동반 이전).
- `core/agent-runtime`에는 provider-neutral 계약만 잔존: `DefaultAgentRunner`,
  `AgentRunner`, `AgentRuntimeProvider`, `AgentRunRequest`, `AgentResumeRequest`,
  `AgentRunResult`, `AgentRunFailureKind`, `ProviderHealth`, `ResolvedRuntimeTarget`.
  검증: `grep -rn "OpenClaw" apps/orchestrator/src/core` → 결과 없음.
- import 방향: 옮긴 provider는 core의 `AgentRuntimeProvider` 인터페이스를 import
  (app→core, 허용 방향). `app/openclaw-ipc.ts`는 이제 OpenClaw 타입을 app-local
  (`./openclaw-provider.js`)에서 가져온다(app→app). 합성 루트는 기존대로 provider를
  생성·주입하며 import 경로만 갱신.
- 행동 불변: model-policy/agent-runtime 단위 + 통합 테스트와 fake-CLI
  `smoke:openclaw` 모두 green.
